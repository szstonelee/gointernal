...
func tryDuckerWithPointer() {
	fmt.Printf("\ntyrDuckerWithPointer ....\n\n")
	...
	v1, ok1 := any.(ducker)
	if ok1 {
		fmt.Printf("any, empty interface, assert interface ducker, type = %T, val = %v\n", v1, v1)
	} else {
		fmt.Printf("any, empty interface, fail to assert interface ducker\n")
	}
	...
	if ok2 {
		...
		var i ducker = v2
		fmt.Printf("assign v2 to ducker interface, type = %T\n", i)
	}
